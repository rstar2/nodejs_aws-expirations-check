...
const login = (body) => {
    ...
    return validateUser(email)
        // .then(user => console.dir(user) || user) // just for tracing
...
const validateUser = (email) => {
    return doGetByEmail(email)
        ...
        .then(user => user.state === 'ACTIVE' ? user : Promise.reject(`User with that email ${email} is not activated yet.`));
};
...
// This is not needed pattern for DynamoDB as the actions over it with 'aws-sdk' imply DB connection
// but if the DB implementation is changed to MongoDB for instance then an active "connection" is needed first
module.exports = () => {
    const db = {
        register,
        login,
